...
let compute_import_data_coverage data_directory data =
  data

let compute_export_data_coverage data_directory sections data =
...
let compute_data_directory_coverage optional_header export_data import_data sections data =
  ...
  | Some header ->
    ...
    let data =
      ...
      | Some data ->
        compute_export_data_coverage dd sections data
      | _ -> data
    in
    match import_data with
    | Some data ->
      compute_import_data_coverage dd data
    | _ -> data
...
let compute_section_table_coverage sections data =
  List.fold_left (fun acc section ->
      ...
      in
      ByteCoverage.add elt acc
    ) data sections

let compute_byte_coverage header size export_data import_data sections binary :ByteCoverage.t =
